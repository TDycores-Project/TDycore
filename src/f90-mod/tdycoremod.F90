...
#include <petsc/finclude/petscvec.h>
  ...
  subroutine TDyRegisterPressureFn(func_name, func, ierr)
    use, intrinsic :: iso_c_binding
    implicit none
    character(len=*), intent(in)  :: func_name
    interface
      subroutine func(tdy, x, pressure, dummy, ierr)
        use tdycoredef
        use petscvec
        TDy :: tdy
      ...
      function RegisterPressureFn(name, func) bind (c, name="TDyRegisterPressureFn") result(ierr)
        use, intrinsic :: iso_c_binding
        implicit none
  ...
  subroutine TDyRegisterTemperatureFn(func_name, func, ierr)
    ...
    character(len=*), intent(in)  :: func_name
    interface
      subroutine func(tdy, x, temperature, dummy, ierr)
        use tdycoredef
        use petscvec
        TDy :: tdy
        PetscReal, intent(in)  :: x(3)
      ...
      function RegisterTemperatureFn(name, func) bind (c, name="TDyRegisterTemperatureFn") result(ierr)
        use, intrinsic :: iso_c_binding
        implicit none
        type(c_ptr), value :: name
        type(c_funptr), value :: func
  ...
  subroutine TDyRegisterVelocityFn(func_name, func, ierr)
    ...
    character(len=*), intent(in)  :: func_name
    interface
      subroutine func(tdy, x, velocity, dummy, ierr)
        use tdycoredef
        use petscvec
        TDy :: tdy
        PetscReal, intent(in)  :: x(3)
        PetscReal, intent(out) :: velocity
        integer :: dummy(*)
        PetscErrorCode :: ierr
      end subroutine

      function RegisterVelocityFn(name, func) bind (c, name="TDyRegisterVelocityFn") result(ierr)
        use, intrinsic :: iso_c_binding
        implicit none
        type(c_ptr), value :: name
        type(c_funptr), value :: func
  ...
  subroutine TDySelectBoundaryPressureFn(tdy, func_name, ierr)
    ...
    integer, intent(out) :: ierr
    interface
      function SelectBoundaryPressureFn(tdy, name, ctx) bind (c, name="TDySelectBoundaryPressureFn") result(ierr)
        use, intrinsic :: iso_c_binding
        implicit none
