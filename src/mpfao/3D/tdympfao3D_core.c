...
/* -------------------------------------------------------------------------- */
PetscErrorCode ComputeTempTransmissibilityMatrix_ForInternalVertex(TDy tdy,
    PetscInt ivertex, TDy_cell *cells) {
  ...
  for (j=0; j<ncells; j++) {
    for (i=0; i<nfluxes; i++) {
      tdy->Temp_Trans[vertex_id][i][j] = CuptimesAinvB_1d[idx] - Fup[i][j];
      if (fabs(tdy->Temp_Trans[vertex_id][i][j])<PETSC_MACHINE_EPSILON) tdy->Temp_Trans[vertex_id][i][j] = 0.0;
      idx++;
    }
  }
  ...
  for (i=0; i<nfluxes; i++) {
    ...
    for (j=0; j<ncells; j++) {
      col = vertices->internal_cell_ids[vOffsetCell + j];
      ierr = MatSetValues(tdy->Temp_Trans_mat,1,&row,1,&col,&tdy->Temp_Trans[vertex_id][i][j],ADD_VALUES); CHKERRQ(ierr);
    }
  }
...
/* -------------------------------------------------------------------------- */
PetscErrorCode ComputeTempTransmissibilityMatrix_ForBoundaryVertex_SharedWithInternalVertices(TDy tdy,
    PetscInt ivertex, TDy_cell *cells) {
  ...
  */
  ...
  for (j=0;j<npcen+npitf_bc;j++) {
    for (i=0;i<nflux_in;i++) {
      if (j<npcen) {
        tdy->Temp_Trans[vertex_id][i][j] = CupInxCBCtimesAinvB_1d[idx] - Fup[i][j];
      } else {
        tdy->Temp_Trans[vertex_id][i][j] = CupInxCBCtimesAinvB_1d[idx] + Cup[i][j-npcen+npitf_in];
      }
      if (fabs(tdy->Temp_Trans[vertex_id][i][j])<PETSC_MACHINE_EPSILON) tdy->Temp_Trans[vertex_id][i][j] = 0.0;
      idx++;
    }
  ...
  for (j=0;j<npcen+npitf_bc;j++) {
    for (i=0;i<nflux_bc;i++) {
      if (j<npcen) {
        tdy->Temp_Trans[vertex_id][i+nflux_in         ][j] = CupBCxCBCtimesAinvB_1d[idx] - Fup[i+nflux_in][j];
        tdy->Temp_Trans[vertex_id][i+nflux_in+nflux_bc][j] = CdnBCxCBCtimesAinvB_1d[idx] - Fdn[i+nflux_in][j];
      } else {
        tdy->Temp_Trans[vertex_id][i+nflux_in         ][j] = CupBCxCBCtimesAinvB_1d[idx] + Cup[i+npitf_in][j-npcen+npitf_in];
        tdy->Temp_Trans[vertex_id][i+nflux_in+nflux_bc][j] = CdnBCxCBCtimesAinvB_1d[idx] + Cdn[i+npitf_in][j-npcen+npitf_in];
      }
      if (fabs(tdy->Temp_Trans[vertex_id][i+nflux_in         ][j])<PETSC_MACHINE_EPSILON) tdy->Temp_Trans[vertex_id][i+nflux_in         ][j] = 0.0;
      if (fabs(tdy->Temp_Trans[vertex_id][i+nflux_in+nflux_bc][j])<PETSC_MACHINE_EPSILON) tdy->Temp_Trans[vertex_id][i+nflux_in+nflux_bc][j] = 0.0;
      idx++;
    }
  }
  ...
  PetscInt num_subfaces = 4;
  for (i=0; i<nflux_in+nflux_bc*2; i++) {
    face_id = vertices->face_ids[vOffsetFace + i];
    subface_id = vertices->subface_ids[vOffsetFace + i];
    row = face_id*num_subfaces + subface_id;
    ...
    for (j=0; j<npitf_bc; j++) {
      col = idxBnd[j] + tdy->mesh->num_cells;
      ierr = MatSetValues(tdy->Temp_Trans_mat,1,&row,1,&col,&tdy->Temp_Trans[vertex_id][i][j+npcen],ADD_VALUES); CHKERRQ(ierr);
    }
  }
...
/* -------------------------------------------------------------------------- */
PetscErrorCode ComputeTempTransmissibilityMatrix_ForBoundaryVertex_NotSharedWithInternalVertices(TDy tdy,
    PetscInt ivertex, TDy_cell *cells) {
  DM             dm;
  TDy_vertex     *vertices;
  TDy_subcell    *subcells;
  ...
  for (iface=0; iface<subcells->num_faces[subcell_id]; iface++) {
    ...
    for (j=0; j<dim; j++) {
      tdy->Temp_Trans[vertices->id[ivertex]][iface][j] = Gmatrix[iface][j];
      if (fabs(tdy->Temp_Trans[vertices->id[ivertex]][iface][j])<PETSC_MACHINE_EPSILON) tdy->Temp_Trans[vertices->id[ivertex]][iface][j] = 0.0;
    }
    tdy->Temp_Trans[vertices->id[ivertex]][iface][dim] = 0.0;
    for (j=0; j<dim; j++) tdy->Temp_Trans[vertices->id[ivertex]][iface][dim] -= (Gmatrix[iface][j]);
  }
  ...
  for (i=0; i<subcells->num_faces[subcell_id]; i++) {
    ...
    for (j=0; j<numBnd; j++) {
      col = idxBnd[j] + tdy->mesh->num_cells;
      ierr = MatSetValues(tdy->Temp_Trans_mat,1,&row,1,&col,&tdy->Temp_Trans[ivertex][i][j],ADD_VALUES); CHKERRQ(ierr);
    }

    icell  = vertices->internal_cell_ids[vOffsetCell + 0];
    col = icell;
    ierr = MatSetValues(tdy->Temp_Trans_mat,1,&row,1,&col,&tdy->Temp_Trans[ivertex][i][numBnd],ADD_VALUES); CHKERRQ(ierr);
  }
...
PetscErrorCode TDyComputeTransmissibilityMatrix3DMesh(TDy tdy) {
  ...
  for (ivertex=0; ivertex<mesh->num_vertices; ivertex++) {
    ...
    if (vertices->num_boundary_cells[ivertex] == 0) {
      ierr = ComputeTransmissibilityMatrix_ForInternalVertex(tdy, ivertex, cells); CHKERRQ(ierr);
      if (tdy->mode == TH) {ierr = ComputeTempTransmissibilityMatrix_ForInternalVertex(tdy, ivertex, cells); CHKERRQ(ierr);}
    } else {
      if (vertices->num_internal_cells[ivertex] > 1) {
        ierr = ComputeTransmissibilityMatrix_ForBoundaryVertex_SharedWithInternalVertices(tdy, ivertex, cells); CHKERRQ(ierr);
        if (tdy->mode == TH) {ierr = ComputeTempTransmissibilityMatrix_ForBoundaryVertex_SharedWithInternalVertices(tdy, ivertex, cells); CHKERRQ(ierr);}

      } else {
        ierr = ComputeTransmissibilityMatrix_ForBoundaryVertex_NotSharedWithInternalVertices(tdy, ivertex, cells); CHKERRQ(ierr);
        if (tdy->mode == TH) {ierr = ComputeTempTransmissibilityMatrix_ForBoundaryVertex_NotSharedWithInternalVertices(tdy, ivertex, cells); CHKERRQ(ierr);}
      }
    }
  }
