...
PetscErrorCode TDyTHInitialize(TDy tdy) {
  ...
  if (tdy->options.init_with_random_field) {
    ...
    ierr = VecGetLocalSize(tdy->soln_prev,&local_size); CHKERRQ(ierr);
    ...
    ierr = VecGetArray(tdy->soln_prev,&soln_p); CHKERRQ(ierr);
    ...
    ierr = VecRestoreArray(temp_vec,&temp_p); CHKERRQ(ierr);
    ierr = VecRestoreArray(tdy->soln_prev,&soln_p); CHKERRQ(ierr);
    ierr = VecDestroy(&temp_vec); CHKERRQ(ierr);
    ierr = PetscRandomDestroy(&rand); CHKERRQ(ierr);
  }
  else {
    ierr = VecSet(tdy->soln_prev,101325.); CHKERRQ(ierr);
  }
