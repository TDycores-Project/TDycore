...
PetscErrorCode TDyTimeIntegratorCreate(TDyTimeIntegrator *_ti)
{
  ...
  *_ti = ti;
  ...
  size_t len;
  ierr = PetscStrlen(time_integration_method, &len);
  CHKERRQ(ierr);
  if (len)
  {
...
PetscErrorCode TDyTimeIntegratorRunToTime(TDy tdy, PetscReal sync_time)
{
  ...
  case TDySNES:
    ...
    MPI_Comm comm;
    PetscMPIInt rank;
    ierr = PetscObjectGetComm((PetscObject)ti->snes, &comm);
    CHKERRQ(ierr);
    ...
    {
      ierr = TDyTimeIntegratorSetTargetTime(ti, sync_time);
      CHKERRQ(ierr);
      ierr = TDySetDtimeForSNESSolver(tdy, ti->dt);
      CHKERRQ(ierr);
      if (!rank)
      {
      ...
      CHKERRQ(ierr);
      ierr = SNESSolve(ti->snes, PETSC_NULL, tdy->soln_prev);
      CHKERRQ(ierr);
      ierr = TDyPostSolveSNESSolver(tdy, tdy->soln_prev);
      CHKERRQ(ierr);
      ti->time += ti->dt;
      ti->istep++;
      PetscInt nit, lit;
      ierr = SNESGetIterationNumber(ti->snes, &nit);
      CHKERRQ(ierr);
      ierr = SNESGetLinearSolveIterations(ti->snes, &lit);
      CHKERRQ(ierr);
      ierr = SNESGetConvergedReason(ti->snes, &reason);
      CHKERRQ(ierr);
      ...
      if (tdy->io->enable_checkpoint)
      {
        ...
        {
          ierr = TDyIOOutputCheckpoint(tdy);
          CHKERRQ(ierr);
          checkpoint = PETSC_FALSE;
        }
      }
      ierr = TDyTimeIntegratorUpdateDT(ti, sync_time);
      CHKERRQ(ierr);
    }
  ...
  case TDyTS:
    ...
    ierr = TSSetTimeStep(ti->ts, ti->dt_init);
    CHKERRQ(ierr);
    ierr = TSSetMaxTime(ti->ts, delta_time);
    CHKERRQ(ierr);
    ierr = TSSolve(ti->ts, tdy->soln_prev);
    CHKERRQ(ierr);
