  ...
  for (ivertex=0; ivertex<mesh->num_vertices; ivertex++) {
    ...
    if (vertices->num_boundary_faces[ivertex] == 0) {
      ...
      for (irow=0; irow<nflux_in; irow++) {
        ...
        if (bcs[face_set].flow_bc.type == VELOCITY_BC) {
    ...
    for (irow=0; irow<ncells; irow++){
      ...
      for (iface=0;iface<subcells->num_faces[subcell_id];iface++) {
        ...
        if (faces->is_internal[face_id] == 0) {
          ...
          PetscInt face_set = 0; // FIXME
          if (bcs[face_set].flow_bc.type == PRESSURE_BC) {
...
PetscErrorCode TDyMPFAO_SetBoundaryTemperature(TDy tdy, Vec Ul) {
  ...
  for (iface=0; iface<mesh->num_faces; iface++) {
    ...
    PetscInt face_set = 0; // FIXME
    if (bcs[face_set].thermal_bc.type == TEMPERATURE_BC) {
...
PetscErrorCode TDyMPFAO_SetBoundarySalineConcentration(TDy tdy, Vec Ul) {
  ...
  for (iface=0; iface<mesh->num_faces; iface++) {
    ...
    PetscInt face_set = 0; // FIXME
    if (bcs[face_set].salinity_bc.type == SALINE_CONC_BC) {
