...
ierr = DMCreateGlobalVector(tdy->dm,&tdy->solution); CHKERRQ(ierr); ierr = DMCreateMatrix(tdy->dm,&tdy->J); CHKERRQ(ierr);
  ...
  /*geh: yes, the code for both SNES implementation is redundant.  It should
         eventually be placed in a separate function */
