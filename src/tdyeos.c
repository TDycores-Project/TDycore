...
PetscErrorCode ComputeWaterDensity_BatzleWang(PetscReal P, PetscReal T, PetscReal S, PetscReal dS, PetscReal d2S, PetscReal *den, PetscReal *dden_dP, PetscReal *dden_dPsi, PetscReal *d2den_dP2) {
  ...
  *dden_dPsi = (0.668 * dS + 0.44 * d2S + 1e-6 * 300 * P * Pa_to_MPa *dS - 1e-6 * 2400. * P*Pa_to_MPa * d2S + 1e-6* T * 80 * dS + 1e-6 * pow(T,2) * 3. * dS - d2S * 1e-6 * T * 3300. - 1e-6 * T * 13. * P*Pa_to_MPa * dS + d2S * 1e-6 * 47 * T * P*Pa_to_MPa) * 1000.;
  *d2den_dP2 = 0.0;
  PetscFunctionReturn(0);
}
